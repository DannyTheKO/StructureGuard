...
public class ChunkLoadListener implements Listener {
    ...
    private void processChunkStructures(World world, int chunkX, int chunkZ) {
        try {
            List<StructureFinder.StructureResult> structures = plugin.getStructureFinder().getStructuresInChunk(world, chunkX, chunkZ);
            ...
            for (StructureFinder.StructureResult structure : structures) {
                ...
                StructureDatabase.StructureInfo existing = plugin.getDatabase().getStructureByChunk(world.getName(), structure.structureType, structure.chunkX, structure.chunkZ);
                if (existing != null) {
                    ...
                    if (grown) {
                        ...
                        StructureDatabase.StructureInfo updated = new StructureDatabase.StructureInfo(world.getName(), structure.structureType, newMinX, newMinZ, newMaxX, newMaxZ, newMinY, newMaxY, structure.chunkX, structure.chunkZ, existing.hasRegion, existing.regionId);
                ...
                final StructureFinder.StructureResult finalStructure = structure; final ConfigManager.ProtectionRule finalRule = rule;
    ...
    private void createProtection(World world, StructureFinder.StructureResult structure, ConfigManager.ProtectionRule rule) {
        try {
            ...
            StructureDatabase.StructureInfo dbInfo = new StructureDatabase.StructureInfo(world.getName(), structure.structureType, structure.minX, structure.minZ, structure.maxX, structure.maxZ, structure.minY, structure.maxY, structure.chunkX, structure.chunkZ, false, null);
    ...
    private long packChunkCoords(int x, int z) { return ((long) x & 0xFFFFFFFFL) | (((long) z) << 32); }
